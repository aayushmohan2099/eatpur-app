...
export default function ProdList() {
  ...
  const fetchProducts = async (page = 1) => {
    ...
    try {
      ...
      setProdCount(res.data?.count || res.count || 0);
  ...
  const formattedData = products.map((prod) => ({
    ...
    checkbox: (
      <div
        className={`p-2 rounded ${prod.is_trending ? "bg-amber-50 border border-amber-200" : ""}`}
      >
        <input
          type="checkbox"
    ...
    productDetails: (
      ...
      >
        {prod.cover_image ? (
          ...
          <div className="w-12 h-12 rounded bg-slate-100 flex items-center justify-center text-slate-400 text-xs shadow-sm border border-slate-200">
            No Img
          </div>
        )}
    ...
    price_info: (
      <div className="flex flex-col">
        {parseFloat(prod.discounted_price) > 0 &&
          parseFloat(prod.discounted_price) !==
            parseFloat(prod.fixed_price) && (
            <span className="font-medium text-[--color-eatpur-dark]">
              ₹{prod.discounted_price}
            </span>
          )}
        <span className="text-xs text-rose-500 font-semibold line-through">
          ₹{prod.fixed_price}
        </span>
      </div>
    ),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between bg-[--color-eatpur-white-warm] p-4 rounded-xl border border-[--color-eatpur-yellow-light]">
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          <label className="px-4 py-2 flex items-center gap-2 text-sm rounded-lg border border-slate-300 font-medium text-slate-700 cursor-pointer">
            Total Products - <strong>{ProdCount}</strong>
          </label>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          <EatpurTable
            ...
            showActions={true}
            onViewClick={(row) =>
              setEditorProduct(products.find((p) => p.id === row.id))
            }
          />
