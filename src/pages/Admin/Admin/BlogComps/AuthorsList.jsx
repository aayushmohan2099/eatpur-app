...
export default function AuthorsList() {
    ...
    const fetchAuthorsData = async (page = 1) => {
        try {
            ...
            setError("Failed to load author analytics. Please check your connection.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const formattedData = authors.map((author) => ({
        ...
        authorProfile: (
            <div className="flex items-center gap-3 py-1">
                {author.avatar ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-[--color-eatpur-green-light] flex items-center justify-center text-white font-bold shadow-sm" style={{ fontFamily: "var(--font-display)" }}>
        ...
        // Standard numbers
        total_blogs: (
            <span className="font-medium text-[--color-eatpur-dark]">{author.total_blogs}</span>
        ),
        ...
        avgComments: (
            <Badge
                text={`${author.avg_comments} Comments`}
                type="info"
                icon="💬"
            />
        ),
    ...
    const handleActionIntercept = (authorData) => {
        alert(`Viewing detailed analytics for ${authorData.username} (ID: ${authorData.id})`);
                ...
                <div className="w-full rounded-xl border border-[--color-eatpur-yellow-light] bg-white overflow-hidden shadow-sm">
                    ...
                    <div className="p-6 space-y-4">
                        {[1, 2, 3, 4].map((skeletonIdx) => (
                            <div key={skeletonIdx} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-none">
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between bg-white px-6 py-4 rounded-xl border border-[--color-eatpur-yellow-light] shadow-sm">
                            <span className="text-sm text-[--color-eatpur-text-light]">
                                Showing Page <span className="font-semibold text-[--color-eatpur-dark]">{currentPage}</span> of {totalPages}
                            </span>
